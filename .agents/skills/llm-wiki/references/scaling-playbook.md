...
`wiki_search.py` provides (`$SCRIPTS` is the `llm-wiki` skill's `scripts/` directory — resolve it as in SKILL.md → "Running the bundled scripts"; use `python3`, falling back to `python` only if absent):

- `python3 "$SCRIPTS/wiki_search.py" "query terms"` — top-N pages by BM25 score.
