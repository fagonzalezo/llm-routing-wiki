...
If the wiki has the optional graph layer (`$SCRIPTS` is the `llm-wiki` skill's `scripts/` directory — resolve it as in SKILL.md → "Running the bundled scripts"; use `python3`, falling back to `python` only if absent):
...
2. Run `python3 "$SCRIPTS/wiki_graph_lint.py" wiki/`. Triage findings with the user before extracting; do not silently rewrite typed edges.
3. Run `python3 "$SCRIPTS/wiki_graph_extract.py" wiki/` to regenerate `nodes.jsonl`, `edges.jsonl`, `graph.sqlite`, `graph.graphml`.
