...
> `$SCRIPTS` is the `llm-wiki` skill's `scripts/` directory — resolve it as in SKILL.md → "Running the bundled scripts". Use `python3` (fall back to `python` only if absent).

```bash
python3 "$SCRIPTS/wiki_lint.py" wiki/
```

If the wiki has the optional graph layer (`wiki/graph/ontology.yaml` exists), also run:

```bash
python3 "$SCRIPTS/wiki_graph_lint.py" wiki/
```
...
- **Orphan pages** — pages with no inbound references from anywhere else in the wiki. A reference is either a body `[[wikilink]]` or a frontmatter slug in a `sources`/`entities`/`concepts` list, so a source page cited by the entity/concept pages it informs is *not* an orphan. Orphans are usually a sign that an ingest forgot to update a parent page. They become invisible because the index-first navigation can't surface them.
...
- **Frontmatter issues** — pages missing required fields (`type`, `title`, `tags`, `created`, `updated`; non-source pages also need `sources`), or with malformed YAML.
...
```bash
python3 "$SCRIPTS/wiki_lint.py" wiki/ --suggest-pages
```
