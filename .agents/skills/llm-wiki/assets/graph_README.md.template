...
> In the commands below, `$SCRIPTS` is the `llm-wiki` skill's `scripts/` directory
> (e.g. `${CLAUDE_PLUGIN_ROOT}/skills/llm-wiki/scripts` in Claude Code, or the
> `scripts/` folder beside the skill's `SKILL.md` in other agents). Resolve it to
> an absolute path first, and use `python3` (fall back to `python` only if absent).

```bash
python3 "$SCRIPTS/wiki_graph_extract.py" wiki/ --out wiki/graph
```
...
2. Run `python3 "$SCRIPTS/wiki_graph_lint.py" wiki/` — catches unknown predicates, broken object references, missing evidence, alias collisions.
3. Run `python3 "$SCRIPTS/wiki_graph_extract.py" wiki/ --out wiki/graph` — regenerates the artifacts above.
4. Query with `python3 "$SCRIPTS/wiki_graph_query.py" wiki/ neighbors --node product:konvy` (or `edges`, `path`, `facts`).
