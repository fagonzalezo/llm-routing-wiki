...
The skill ships Python helper scripts in its own `scripts/` directory. They are **not** in the project you are working in — resolve their location once per session and reuse it. Pick the first option that applies:

- **Claude Code:** `SCRIPTS="${CLAUDE_PLUGIN_ROOT}/skills/llm-wiki/scripts"`.
- **Other harnesses (Codex, Antigravity, OpenCode, generic agentskills.io):** the `scripts/` directory next to *this* `SKILL.md`. You were given this skill's absolute path when it loaded — use it: `SCRIPTS="<skill-dir>/scripts"`.
- **Fallback (location unknown):** locate it — `SCRIPTS="$(dirname "$(find . ~/.claude/skills .claude/skills -name wiki_lint.py 2>/dev/null | head -1)")"`.

Then invoke every script through `$SCRIPTS` with `python3` (fall back to `python` only if `python3` is absent), e.g. `python3 "$SCRIPTS/wiki_lint.py" wiki/`. Throughout this skill and its reference files, `python3 "$SCRIPTS/..."` always means the path you resolved here, and the working directory is assumed to be the project root (the parent of `wiki/`). The scripts are pure stdlib and run on **Python 3.9+**; the three graph scripts additionally require PyYAML (`pip install pyyaml`).
...
```bash
python3 "$SCRIPTS/wiki_graph_lint.py" wiki/      # check ontology + evidence + alias collisions
python3 "$SCRIPTS/wiki_graph_extract.py" wiki/   # rebuild nodes.jsonl, edges.jsonl, graph.sqlite, graph.graphml
python3 "$SCRIPTS/wiki_graph_query.py" wiki/ neighbors --node product:konvy
```
...
**YAML frontmatter on every page.** Every wiki page begins with frontmatter that includes `type`, `title`, `tags`, `created`, and `updated`; non-source pages additionally carry `sources`. The bundled `wiki_search.py` script can filter on these without reading page bodies. See `references/page-conventions.md`.
...
**Search script for large wikis.** Once the wiki passes ~300 pages, plain index lookup may not surface the right pages for fuzzy queries. Use `python3 "$SCRIPTS/wiki_search.py"` for BM25-ranked retrieval with optional frontmatter filters. It's a fallback, not the default — index-first is still cheaper when it works.
...
```bash
python3 "$SCRIPTS/init_wiki.py" <project-root> [--wiki-dir wiki] [--raw-dir raw]
```
