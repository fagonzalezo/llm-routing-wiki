...
Usage:
    python3 wiki_lint.py [<wiki-dir>] [options]

Options:
    --soft-cap N        Page-size soft cap in lines (default: 400)
    --hard-cap N        Page-size hard cap in lines (default: 800)
    --required-fm a,b   Base required frontmatter fields for every page
                        (default: type,title,tags,created,updated). Non-source
                        pages additionally require `sources`.
...
Examples:
    python3 wiki_lint.py wiki/
    python3 wiki_lint.py wiki/ --suggest-pages
    python3 wiki_lint.py wiki/ --json > lint.json
"""
...
# Resolved as inbound links alongside body [[wikilinks]] (see page-conventions.md).
FRONTMATTER_REF_FIELDS = ("sources", "entities", "concepts")


def frontmatter_refs(meta: dict) -> list[str]:
    """Flatten the slug references declared in frontmatter ref fields."""
    refs = []
...
def lint(pages: list[dict], soft_cap: int, hard_cap: int, required_fm: list[str], suggest_pages: bool, suggest_min: int) -> dict:
    ...
    # Inbound link map. Body [[wikilinks]] plus frontmatter slug references
    # (sources/entities/concepts) both count as inbound links — this is what
    # keeps source pages, which are cited via frontmatter rather than body
    # wikilinks, from being reported as orphans.
    inbound = defaultdict(set)
    all_slugs = set(slug_to_pages.keys())
    for p in pages:
        for ref in set(p["links"]) | set(p["fm_refs"]):
            inbound[ref].add(p["slug"])
    ...
    for p in pages:
        ...
        # Frontmatter. Every page needs the base required fields; non-source
        # pages additionally need `sources` (a source page is itself the source).
        if p["malformed_fm"]:
            findings["malformed_frontmatter"].append({"path": p["rel_path"]})
        else:
            effective_required = list(required_fm)
            if p["meta"].get("type") != "source" and "sources" not in effective_required:
                effective_required.append("sources")
            missing = [field for field in effective_required if field not in p["meta"] or p["meta"].get(field) in ("", None, [])]
            if missing:
...
def main():
    ...
    parser.add_argument("--required-fm", default="type,title,tags,created,updated",
                        help="Base required frontmatter fields, comma-separated. "
                             "Non-source pages additionally require `sources`.")
