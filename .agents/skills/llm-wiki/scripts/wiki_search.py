...
Usage:
    python3 wiki_search.py "query terms" [options]
...
Examples:
    python3 wiki_search.py "diffusion training stability" --top 5
    python3 wiki_search.py "alignment" --type concept --tag safety
    python3 wiki_search.py "" --backlinks transformer
    python3 wiki_search.py "" --top-linked 10
"""
...
# Counted as links alongside body [[wikilinks]] (see page-conventions.md).
...
def collect_pages(wiki_root: Path) -> list[dict]:
    ...
    for md_path in wiki_root.rglob("*.md"):
        # Skip the schema, index, log, readme, and template files
        rel = md_path.relative_to(wiki_root)
        if rel.parts[0] in {"SCHEMA.md", "index.md", "log.md", "README.md"} or rel.name.startswith("."):
            continue
...
def cmd_backlinks(args, pages: list[dict]) -> None:
    ...
    inbound = []
    for page in pages:
        if target in page["links"] or target in page["fm_refs"]:
            inbound.append(page)
    if not inbound:
...
def cmd_top_linked(args, pages: list[dict]) -> None:
    ...
    for page in pages:
        for link in set(page["links"]) | set(page["fm_refs"]):
